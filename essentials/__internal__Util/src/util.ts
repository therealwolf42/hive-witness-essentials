 ...
 */
export let failover_node = (rpc_nodes: Array<string>, current_rpc_node: string) => {
 ...
 */
export let order_keys = (signing_keys: Array<{ public: string, private: string }>, key: string) => {
  for(let k of signing_keys) {
    if(k.public === key) {
      let i = signing_keys.indexOf(k)
 ...
 */
export let get_next_key = (signing_keys: Array<{ public: string, private: string }>, key: string, infinite = false) : { public: string, private: string} => { 
  let keypair = signing_keys.filter(x => x.public === key)[0]
  let i = signing_keys.indexOf(keypair)
 ...
 */
export let choose_transaction_key = (current_key:string, active_key:string, signing_keys: Array<{ public: string, private: string }> = []) => {
  if (signing_keys.length > 0) {
    if(current_key.slice(-39) === '1111111111111111111111111111111114T1Anm') {
      return active_key ? active_key : ''
    }
    for (let key of signing_keys) {
...
export let convert_nai_precision = (amount, precision) => {
