...
export const cmd_update_key = async (
  key: string,
  node: string,
  rotate = false,
) => {
  ...
  if (!key && rotate) {
    const next_key = essentials.get_next_key(
  ...
  if (!transaction_signing_key) {
    ...
    if (!transaction_signing_key)
      return console.error(
        `Invalid Signing Key Pairs in config. Or witness is disabled, which requires your private active key.`,
      )
  } else {
  ...
  await update_witness(_g.CURRENT_SIGNING_KEY, transaction_signing_key, props, {
    set_properties,
  })
  return key
}
