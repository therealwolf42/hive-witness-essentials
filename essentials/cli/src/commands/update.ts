...
const props: any = {new_signing_key: ''}
const start = async () => {
  const node = process.argv[2]
  const res = await get_witness({node})
  ...
  await update_witness(res.signing_key, transaction_signing_key, props, {
    set_properties,
  })
...
const read = () => {
  const witness_url = readline.question(
    `witness_url? [${_g.witness_data.url}] : `,
  )
  const account_creation_fee = readline.question(
    `account_creation_fee? (number only - without STEEM) [${_g.witness_data.props.account_creation_fee}] : `,
  )
  ...
  const account_subsidy_budget = Number(
    readline.question(
      `account_subsidy_budget? [${_g.witness_data.props.account_subsidy_budget}] : `,
    ),
  )
  const account_subsidy_decay = Number(
    readline.question(
      `account_subsidy_decay? [${_g.witness_data.props.account_subsidy_decay}] : `,
    ),
  )
  const new_signing_key = readline.question(
    `new_signing_key? [${_g.CURRENT_SIGNING_KEY}] : `,
  )
  ...
  if (account_creation_fee && !isNaN(Number(account_creation_fee))) {
    props.account_creation_fee = _g.witness_data.props.account_creation_fee = `${Number(
      account_creation_fee,
    ).toFixed(3)} STEEM`
  }
  if (maximum_block_size && !isNaN(Number(maximum_block_size))) {
    props.maximum_block_size = _g.witness_data.props.maximum_block_size = Number(
      maximum_block_size,
    )
  }
  if (
