...
export const start_witness_watcher = async (
  IS_TESTING = false,
  NOTIFY = true,
  FAILOVER = true,
  DISABLE = true,
  ...
  try {
    while (true) {
      const b = await watch_witness(IS_TESTING, NOTIFY, FAILOVER, DISABLE)
...
const watch_witness = async (IS_TESTING, NOTIFY, FAILOVER, DISABLE) => {
  try {
    ...
    } = await essentials.get_witness_by_account(
      _g.client,
    ...
    if (
      _g.MISSED_BLOCK_FLAG &&
      witness.last_confirmed_block_num > _g.last_confirmed_block_num
    ) {
      essentials.log(`Recovered on block ${witness.last_confirmed_block_num}`)
      await send_alerts(
        `Witness ${_g.witness_data.witness}: Recovered on block ${witness.last_confirmed_block_num}`,
        `Witness ${_g.witness_data.witness}: Recovered on block ${witness.last_confirmed_block_num}`,
      )
    ...
    if (witness.total_missed > _g.current_total_missed) {
      return await handle_missed_block(
        witness,
        IS_TESTING,
        NOTIFY,
        FAILOVER,
...
const initiate_watcher = async (IS_TESTING) => {
  if (_g.start_total_missed === 99999) {
    console.log('\n----------------------------\n')
    console.log(`Initiating Witness Watcher`)

    const x = await essentials.get_witness_by_account(
      _g.client,
      _g.witness_data.witness,
    )
    if (x) {
      ...
      console.log(
        `Witness: ${_g.witness_data.witness} | Current Total Missed Blocks: ${_g.start_total_missed} | Threshold: ${_g.config.MISSED_BLOCKS_THRESHOLD}`,
      )
      console.log(
        `TELEGRAM: ${
          _g.config.TELEGRAM.ENABLED ? 'ENABLED' : 'DISABLED'
        } | EMAIL: ${_g.config.EMAIL.ENABLED ? 'ENABLED' : 'DISABLED'} | SMS: ${
          _g.config.SMS.NEXMO || _g.config.SMS.TWILIO ? 'ENABLED' : 'DISABLED'
        }`,
      )
      console.log(
        `Signing Keys: ${_g.config.SIGNING_KEYS.map((x) => x.public).join(
          ', ',
        )}`,
      )
      console.log(
        `Next Backup Key: ${
          _g.CURRENT_BACKUP_KEY.public !== _g.NULL_KEY
            ? _g.CURRENT_BACKUP_KEY.public
            : `No Backup Keys - Disabling Witness Directly`
        }`,
      )
      console.log(
        `KEY ROTATION: ${
          _g.config.ROTATE_KEYS && _g.config.ROTATE_ROUNDS !== 0
            ? `ENABLED (${
                _g.config.ROTATE_ROUNDS > -1
                  ? _g.config.ROTATE_ROUNDS
                  : 'INFINITE'
              } ROUNDS)`
            : `DISABLED`
        } ${_g.config.TEST_MODE ? 'Test-Mode: ENABLED' : ''}`,
      )
      console.log('\n----------------------------\n')
...
const handle_missed_block = async (
  witness,
  IS_TESTING,
  NOTIFY,
  FAILOVER,
  ...
  const missed_since_start = witness.total_missed - _g.start_total_missed
  ...
  if (
    NOTIFY &&
    (_g.config.ALERT_AFTER_EVERY_MISSED ||
      missed_since_start > _g.config.MISSED_BLOCKS_THRESHOLD)
  ) {
    await send_alerts(
      `Witness ${_g.witness_data.witness}: Missed Block!`,
      `Witness ${_g.witness_data.witness}: Missed Block!${
        missed_since_start < _g.config.MISSED_BLOCKS_THRESHOLD
          ? ` ${missed_since_start} more until failover.`
          : ''
      }`,
    )
  } else {
    essentials.log(
      !NOTIFY
        ? 'TEST-MODE: Would have send notifications for missed block'
        : `Didn't send notification due to not reaching missed block threshold`,
    )
  }
  ...
  if (missed_since_start >= _g.config.MISSED_BLOCKS_THRESHOLD) {
    ...
    if (FAILOVER && (!_g.config.TEST_MODE || IS_TESTING)) {
      const transaction_key = essentials.choose_transaction_key(
        witness.signing_key,
        _g.config.ACTIVE_KEY,
        _g.config.SIGNING_KEYS,
      )
      const props: any = {new_signing_key: _g.CURRENT_BACKUP_KEY.public}
      await update_witness(witness.signing_key, transaction_key, props, {
        set_properties: Boolean(transaction_key !== _g.config.ACTIVE_KEY),
      })
    } else {
      essentials.log(
        `TEST-MODE: Would have updated witness to ${_g.CURRENT_BACKUP_KEY.public}`,
      )
    }
    ...
    if (NOTIFY) {
      await send_alerts(
        `Witness ${_g.witness_data.witness}: Updated Signing Key`,
        `Witness ${_g.witness_data.witness}: Updated Signing Key to ${_g.CURRENT_BACKUP_KEY.public}`,
      )
    } else {
      essentials.log(
        `TEST-MODE: Would have send notifications for updated signing key`,
      )
    }
